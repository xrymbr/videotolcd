...
static int V4l2InitDevice(char *strDevName, PT_VideoDevice ptVideoDevice)
{
	...
	{
		tFmtDesc.pixelformat = V4L2_PIX_FMT_MJPEG;		
        if (isSupportThisFormat(tFmtDesc.pixelformat))
        {
            ptVideoDevice->iPixeformat = tFmtDesc.pixelformat;
            break;
        }
		tFmtDesc.index++;
	}

    if (!ptVideoDevice->iPixeformat)
    {
    ...
    tV4l2Fmt.fmt.pix.pixelformat = ptVideoDevice->iPixeformat;
...
static int V4l2GetFrameForStreaming(PT_VideoDevice ptVideoDevice, PT_VideoBuf ptVideoBuf)
{
    ...
    ptVideoBuf->iPixelFormat        = ptVideoDevice->iPixeformat;
    ptVideoBuf->tPixelDatas.iWidth  = ptVideoDevice->iWidth;
    ptVideoBuf->tPixelDatas.iHeight = ptVideoDevice->iHeight;
    ptVideoBuf->tPixelDatas.iBpp    = (ptVideoDevice->iPixeformat == V4L2_PIX_FMT_YUYV) ? 16 : \
                                        (ptVideoDevice->iPixeformat == V4L2_PIX_FMT_MJPEG) ? 0 :  \
                                        (ptVideoDevice->iPixeformat == V4L2_PIX_FMT_RGB565) ? 16 :  \
                                        0;
...
{
    return ptVideoDevice->iPixeformat;
}
